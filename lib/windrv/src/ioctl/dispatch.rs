...
use log::info;
use spin::Mutex;
use vck_common::{types::Guid, EncryptedOffsetStore, SectorIo, VckError, VckResult, VolumeId};
...
use super::types::{
    BenchAesReq, BenchAesResp, JvckVolumeAttachReq, JvckVolumeAttachResp, JvckVolumePrepareReq,
    JvckVolumePrepareResp, ProgressEvent, VolumeListEntry, VolumeListResponse, VolumeRequest,
    VolumeStatus,
};
...
fn handle_start_encrypt(registry: &VolumeAttachRegistry, input: &[u8]) -> VckResult<IoctlResponse> {
    let req: VolumeRequest = decode_req(input)?;
    ...
    volume
        .encryption
        .lock()
        .start_encrypt(&*volume.offset_store);
    unsafe { crate::filter::volume_thread::wake_for(&volume) };
    encode_resp(&EmptyResponse {})
}

fn handle_start_decrypt(registry: &VolumeAttachRegistry, input: &[u8]) -> VckResult<IoctlResponse> {
    let req: VolumeRequest = decode_req(input)?;
    let volume = resolve_volume(registry, &req.volume_path)
        .ok_or(VckError::NotFound("volume is not attached"))?;
    volume
        .encryption
        .lock()
        .start_decrypt(&*volume.offset_store);
    unsafe { crate::filter::volume_thread::wake_for(&volume) };
    encode_resp(&EmptyResponse {})
}

fn handle_get_progress(registry: &VolumeAttachRegistry, input: &[u8]) -> VckResult<IoctlResponse> {
    let req: VolumeRequest = decode_req(input)?;
    let volume = resolve_volume(registry, &req.volume_path)
        .ok_or(VckError::NotFound("volume is not attached"))?;
    let snapshot = volume.encryption.lock().snapshot();
...
fn handle_jvck_prepare_adddevice_path(
    ...
    let geom_io = KernelVolumeIo::open_query(&io_volume_id).map_err(|e| {
        info!("jvck_prepare(add): geom open failed: {}", e);
        e
    })?;
    ...
    if !req.metadata_block.is_empty() {
        ...
        for lba in &replica_lbas {
            lo.write_sectors(*lba, &sector_buf).map_err(|e| {
                info!("jvck_prepare(add): metadata write lba={} err: {}", lba, e);
                e
            })?;
            info!("jvck_prepare(add): wrote metadata lba={}", lba);
        }
    }
    ...
    let volume = Arc::new(AttachedVolume {
        ...
        io_config: IoConfig::Encrypted {
            ...
            offset_store: Arc::new(DummyOffsetStore {
                total_sectors: data_sectors,
            }),
        },
        encryption: Mutex::new(EncryptionEngine::new(
            offset_sector,
            vck_common::types::EncryptedOffset {
                sector: 0,
                total_sectors: data_sectors,
            },
        )),
        ...
        sweep_io: Mutex::new(Arc::new(DummySectorIo {
            sector_size,
            total_sectors: data_sectors,
        })),
    ...
    info!("jvck_prepare(add): provisional volume bound (AddDevice path)");
        ...
        if let Some(h) = open_volume_handle_raw(&nt_path) {
            ...
            unsafe {
                ...
                let _ = ZwDeviceIoControlFile(
                    ...
                    null_mut(),
                    0,
                    partition_info.as_mut_ptr().cast(),
                    64,
                );
                let _ = ZwClose(h);
            }
    ...
    if !req.vmk.is_empty() {
        ...
        let sweep_io: Arc<dyn SectorIo> =
            Arc::new(LowerDeviceIo::new(lower_do, store_bps, store_data));
        registry.remove(&req.volume_path);
...
impl EncryptedOffsetStore for DummyOffsetStore {
    ...
    fn store(&self, _: &vck_common::types::EncryptedOffset) -> VckResult<()> {
        Ok(())
    }
...
fn handle_jvck_prepare(registry: &VolumeAttachRegistry, input: &[u8]) -> VckResult<IoctlResponse> {
    ...
    let (sector_size, total_sectors): (u32, u64) = vol_handle.map_or((512, 0), |h| {
        ...
        if crate::nt::nt_success(unsafe {
            ZwDeviceIoControlFile(
                ...
                null_mut(),
                &mut iosb,
                IOCTL_DISK_GET_DRIVE_GEOMETRY_PREPARE,
                null_mut(),
                0,
                geom.as_mut_ptr().cast(),
                32,
            )
        }) {
        ...
        if crate::nt::nt_success(unsafe {
            ZwDeviceIoControlFile(
                ...
                null_mut(),
                &mut iosb,
                IOCTL_DISK_GET_LENGTH_INFO_PREPARE,
                null_mut(),
                0,
        ...
        info!("jvck_prepare: geom bps={} total={}", bps, total);
        (bps, total)
    });
    ...
    if sector_size == 0 || total_sectors == 0 {
        if held_lock {
            if let Some(h) = vol_handle {
                send_fsctl(h, FSCTL_UNLOCK_VOLUME);
            }
        }
        if let Some(h) = vol_handle {
            unsafe {
    ...
    let target_do: PDO = if let Some(h) = vol_handle {
        ...
        let st = unsafe {
            ObReferenceObjectByHandle(
                ...
                null_mut(),
                0,
                (&mut fobj as *mut PFILE_OBJECT).cast(),
                null_mut(),
            )
        };
    ...
    info!("jvck_prepare: target_do={:p}", target_do);
    ...
    .map_err(|err| {
        info!("jvck_prepare: attach failed: {}", err);
        if held_lock {
            if let Some(h) = vol_handle {
                send_fsctl(h, FSCTL_UNLOCK_VOLUME);
            }
    ...
    registry.insert(volume.clone());
    unsafe { crate::filter::filter_bind_volume(filter_do, volume.clone()) };
    info!("jvck_prepare: provisional volume bound, size hiding active");

    // ── Step 5: unlock → FSD re-mounts above our filter ─────────────────────
    if held_lock {
        if let Some(h) = vol_handle {
            let st = send_fsctl(h, FSCTL_UNLOCK_VOLUME);
            info!("jvck_prepare: UNLOCK=0x{:08x}", st);
        }
    }
    if let Some(h) = vol_handle {
        unsafe {
    ...
    if !req.metadata_block.is_empty() {
        ...
        if req.metadata_block.len() < METADATA_BLOCK_SIZE {
            ...
            return Err(VckError::InvalidData(
                "metadata_block must be at least 512 bytes",
            ));
        }
        ...
        match KernelVolumeIo::open(&write_vol_id, sector_size, total_sectors) {
            Ok(write_io) => {
                ...
                for lba in &replica_lbas {
                    match write_io.write_sectors(*lba, &sector_buf) {
                        Ok(()) => info!("jvck_prepare: wrote metadata lba={}", lba),
                        Err(err) => {
                            info!("jvck_prepare: write failed lba={}: {}", lba, err);
                            registry.remove(&req.volume_path);
                            crate::filter::detach_filter(filter_do);
                            return Err(err);
                        }
            ...
            Err(err) => {
                info!("jvck_prepare: write_io open failed: {}", err);
                registry.remove(&req.volume_path);
                crate::filter::detach_filter(filter_do);
                return Err(err);
            }
    ...
    let (raw_partition_path, raw_disk_path, partition_start_lba) = {
        ...
        if let Some(h) = open_volume_handle_raw(&nt_path) {
            ...
            if crate::nt::nt_success(st2) {
                ...
                start_offset_bytes =
                    u64::from_le_bytes(partition_info[4..12].try_into().unwrap_or([0; 8]));
            }
            info!(
    ...
    if !req.vmk.is_empty() {
        ...
        let probe_io = KernelVolumeIo::open_query(&io_volume_id).map_err(|err| {
            info!("jvck_prepare: probe failed: {}, detaching", err);
            registry.remove(&req.volume_path);
            crate::filter::detach_filter(filter_do);
            err
        })?;
        ...
        info!("jvck_prepare: probe ok total={}", partition_total);
        ...
        let io_config = on_attach_volume(&req.vmk, None, probe_arc).map_err(|err| {
            info!("jvck_prepare: on_attach failed: {}, detaching", err);
            registry.remove(&req.volume_path);
            crate::filter::detach_filter(filter_do);
            err
        })?;
        ...
        let sweep_io: Arc<dyn SectorIo> = if !raw_disk_path.is_empty() && partition_start_lba > 0 {
            let disk_id = VolumeId {
                partition_guid: Guid::nil(),
                device_path: raw_disk_path.clone(),
            };
            ...
            let part_id = VolumeId {
                partition_guid: Guid::nil(),
                device_path: raw_partition_path.clone(),
            };
            info!(
                "jvck_prepare: sweep via raw partition {}",
                raw_partition_path
            );
            Arc::new(
...
fn handle_jvck_attach(registry: &VolumeAttachRegistry, input: &[u8]) -> VckResult<IoctlResponse> {
    ...
    let provisional = registry.get(&req.volume_path).ok_or(VckError::NotFound(
    ...
    let probe_io = KernelVolumeIo::open_query(&io_volume_id).map_err(|err| {
        info!("jvck_attach: probe open_query failed: {}", err);
        err
    })?;
    let partition_total = probe_io.total_sectors();
    info!(
    ...
    let probe_arc: Arc<dyn SectorIo> = Arc::new(probe_io);
    let io_config = on_attach_volume(&req.vmk, None, probe_arc).map_err(|err| {
        info!("jvck_attach: on_attach failed: {}", err);
        err
    })?;
    info!("jvck_attach: metadata opened");

    let (offset_sector, encrypted_offset, offset_store) = io_config.geometry().ok_or(
        VckError::ValidationFailed("on_attach returned passthrough for a VMK-provided volume"),
    )?;
    let store_data_sectors = encrypted_offset.total_sectors;

    info!("jvck_attach: build sweep_io");
    ...
    let sweep_io: Arc<dyn SectorIo> = if !req.raw_partition_path.is_empty() {
        ...
        info!(
            "jvck_attach: sweep_io via raw partition {}",
            req.raw_partition_path
        );
        Arc::new(KernelVolumeIo::open(
            &part_id,
            store_sector_size,
            store_data_sectors,
        )?)
    } else {
        info!("jvck_attach: sweep_io via volume (no raw path)");
        Arc::new(KernelVolumeIo::open(
            &io_volume_id,
            store_sector_size,
            store_data_sectors,
    ...
    info!("jvck_attach: volume complete and rebound, done");
...
pub fn detach_volume_with_dismount(
    ...
    if let Some(h) = vol_handle {
        ...
        if !dismounted {
            info!(
                "detach: DISMOUNT_VOLUME failed after {} retries",
                MAX_RETRIES
            );
        }
        ...
        if held_lock {
            ...
            info!("detach: UNLOCK_VOLUME=0x{:08x}", unlock_st);
        }
        unsafe {
            let _ = ZwClose(h);
        }
...
fn handle_detach(registry: &VolumeAttachRegistry, input: &[u8]) -> VckResult<IoctlResponse> {
    ...
    if let Some(volume) = registry.get(&req.volume_path) {
        if volume.is_os_volume() && volume.has_encrypted_data() {
            info!(
                "detach: refused — OS volume is encrypted ({})",
                req.volume_path
            );
...
fn handle_pause_os_volume(registry: &VolumeAttachRegistry) -> VckResult<IoctlResponse> {
    for volume in registry.all() {
        if volume.is_os_volume() {
            volume.encryption.lock().pause();
            info!("pause_os_volume: paused {}", volume.volume_path);
        }
    }
    Ok(Vec::new())
}
...
fn handle_detach_all_volumes(registry: &VolumeAttachRegistry) -> VckResult<IoctlResponse> {
    ...
    for path in data_paths {
        info!("detach_all_data: detaching {}", path);
        ...
        if let Err(e) = detach_volume_with_dismount(registry, &path, true, false) {
            info!("detach_all_data: {} failed: {}", path, e);
        }
    }
    Ok(Vec::new())
}
