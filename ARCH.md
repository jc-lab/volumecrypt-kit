...
- metadata가 볼륨 안에 저장되므로 UEFI 로더는 UEFI 변수 핸드오버로 **VMK만** 드라이버에 전달합니다.
...
- JvckMetadata replica는 OS 볼륨 끝부분 footer 영역에 저장합니다. 로더는 부팅 시 이를 Block IO로 읽어
  ...
  **투명하게(transparent) 암·복호화**합니다. 드라이버에는 VMK만 UEFI 변수 핸드오버로 전달합니다.
...
│   └── loader/                  # UEFI 로더 프레임워크 (Block IO 후킹, UEFI 변수 핸드오버)
│
├── sdk/                         # Go: 유저스페이스 SDK
...
| 언어 | 범위 | 빌드 단위 |
|---|---|---|
| Rust | `lib/`, `sample/common`, `sample/crypto-test`, `sample/windrv`, `sample/loader` | Cargo workspace |
...
- UEFI→Driver 핸드오버 추상화
  - `HandoverPayload` 트레이트: `messagepack-serde`(no_std) 직렬화/역직렬화 + 전송용 UEFI 변수
    식별자(`VAR_NAME`/`VAR_GUID`) 연관 상수. 프레임워크는 이 상수를 제네릭하게 읽고, 구체 값은
    integrator(sample)가 지정합니다.
  - **전송 방식: UEFI 런타임 변수**. 로더가 `SetVariable`(`BOOTSERVICE_ACCESS|RUNTIME_ACCESS`)로
    msgpack payload를 발행하고, 드라이버가 `ExGetFirmwareEnvironmentVariable`로 읽어 `decode_payload`로
    역직렬화. 변수 이름/GUID(`VckHandover`, 벤더 GUID)는 **sample**(`vck-sample-common`의
    `VckHandoverPayload` impl)에 정의. (VM 부팅 검증 완료)
  - 과거 ACPI 커스텀 테이블/XSDT 주입(`AcpiHandoverWriter`/`AcpiHandoverReader`) 방식은 **제거됨**:
    커널 `ZwQuerySystemInformation(SystemFirmwareTableInformation,"ACPI")`가 `STATUS_NOT_IMPLEMENTED`라
    드라이버가 ACPI 테이블을 읽을 수 없었기 때문.
- 공통 상수 및 유틸리티
...
│   └── handover/
│       ├── mod.rs
│       └── payload.rs     # HandoverPayload trait + encode/decode_payload
└── Cargo.toml
```

> `EncryptedOffsetStore`/`SectorIo` 트레이트는 의존성 그래프상 `lib/common`(`store.rs`)에 두고
> `lib/windrv`가 재노출합니다. `JvckMetadataStore`는 `SectorIo`에 대해 generic이며, 커널은
> `lib/windrv`의 `KernelVolumeIo`, UEFI는 `lib/common`의 `UefiBlockIoVolume`(uefi feature)를 사용합니다.
...
pub trait HandoverPayload: Serialize + DeserializeOwned {
    /// 페이로드를 싣는 UEFI 변수 이름 (sample에서 지정)
    const VAR_NAME: &'static str;
    /// VAR_NAME의 벤더 GUID (Windows/UEFI GUID 16바이트, sample에서 지정)
...
│   └── handover.rs         # read_handover: UEFI 변수 읽어 payload 역직렬화
└── Cargo.toml
```
...
│         │                                                       │
│    lib/windrv: 볼륨에서 JVCK metadata(header/footer) 탐색·검증   │
...
/// lib/windrv는 이 타입으로 핸드오버 페이로드를 직접 역직렬화하여 `AttachContext`에
...
pub enum IoConfig {
    ...
    /// 고수준: lib/windrv 내부에서 AES-XTS로 자동 처리
    AesXts {
        key1: [u8; 32],
        key2: [u8; 32],
        offset_sector: u64,
...
`lib/windrv`는 `IoCompletion` 콜백과 커널 스레드풀(`ExWorkItem`)을 기반으로
...
// lib/windrv/src/ioctl/codes.rs 와 동일한 값
const (
...
// 키는 Attach(또는 OS Volume의 경우 핸드오버)에서 이미 설정되어 있습니다.
...
핸드오버 페이로드를 소비한 직후 로더/드라이버는 핸드오버 변수 버퍼와 지역 복사본의 VMK를 **zeroize**합니다
(아래 "키 수명·zeroize" 참조).
...
impl HandoverPayload for VckHandoverPayload {
    const VAR_NAME: &'static str = "VckHandover";
    const VAR_GUID: [u8; 16] = /* {59564B43-...} 벤더 GUID 16바이트 */;
}
```

**키 수명·zeroize:** 핸드오버 UEFI 변수 값은 평문 VMK를 담으므로, 드라이버는 부팅 시 VMK를 자체 보호
메모리로 복사한 뒤 변수 읽기 버퍼(NonPagedPool)를 즉시 zeroize하고, 로더 측 지역 변수(VMK·FVEK)도 사용 후
zeroize합니다. FVEK는 핸드오버에 싣지 않습니다.
...
impl VolumeProvider for VckVolumeProvider {
    ...
    async fn on_attach(&self, ctx: &AttachContext<'_, VckHandoverPayload>) -> VckResult<IoConfig> {
        // 1. 핸드오버 페이로드 확보 (lib/windrv가 이미 VckHandoverPayload로 역직렬화)
...
[sample/loader]
  ...
  4. lib/loader: VckHandoverPayload(partition_guid, vmk) → msgpack 직렬화
                 UEFI 런타임 변수(VAR_NAME/VAR_GUID)로 SetVariable 발행
                 로더 지역 FVEK/VMK 사본 사용 후 zeroize
  5. msbootmgfw.os.efi 체인로드 → Windows Boot Manager 기동
      │
      ▼
[Windows Kernel Boot]
  sample/windrv DriverEntry
  1. lib/windrv: ExGetFirmwareEnvironmentVariable로 핸드오버 변수 읽기 →
     VckHandoverPayload(partition_guid, vmk) 역직렬화
     VMK를 보호 메모리로 복사 후 변수 버퍼 zeroize (EFI 접근 없음)
  ...
  4. 이후 모든 볼륨 I/O: lib/windrv가 비동기 파이프라인으로 처리
...
I/O 라우팅, 암·복호화 파이프라인, 핸드오버 직렬화/UEFI 변수 발행·읽기 등 모든 메커니즘은 lib에 위치합니다.
(핸드오버 변수 이름/GUID·페이로드 구조는 sample이 `HandoverPayload`로 지정.)
...
`sdk`는 Go 관점에서의 드라이버 인터페이스 명세이며, `lib/windrv/src/ioctl/`은
...
**핸드오버 확장성:** UEFI 변수 이름/GUID와 msgpack 페이로드 구조는 sample에서 결정합니다
(`HandoverPayload::VAR_NAME`/`VAR_GUID` + 페이로드 타입). `LoaderProvider::Payload`와
`VolumeProvider::Payload`를 동일한 `HandoverPayload` 구현 타입으로 지정하면, lib/loader는 그 타입으로
직렬화·`SetVariable` 발행하고 lib/windrv는 그 변수를 읽어 역직렬화하여 `AttachContext`에 구체 타입으로
전달합니다(`dyn Any` downcast 불필요). lib은 직렬화·UEFI 변수 발행/읽기 헬퍼만 제공하므로 다른 sample이
독립적인 핸드오버 스킴을 가질 수 있습니다.
