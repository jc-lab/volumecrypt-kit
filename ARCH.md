...
/// lib/windrv는 이 타입으로 ACPI 핸드오버 버퍼를 직접 역직렬화하여 `AttachContext`에
...
[Windows Kernel Boot]
  ...
  1. lib/windrv: ACPI 테이블(VCKD) 탐색 → VckHandoverPayload(partition_guid, vmk) 역직렬화
...
지정하면, lib/loader는 그 타입으로 직렬화하고 lib/windrv는 그 타입으로 역직렬화하여
