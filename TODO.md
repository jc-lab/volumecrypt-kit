...
현재 상태: **OS 볼륨 최초 암호화 → 로더 경유 부팅 → 런타임 복호화 전체 경로 구현·VM 검증 완료**
(`make test-vm-os-encrypt` 22/22 통과). JVCK 암호 코어(호스트 단위테스트 31개 통과), 커널 드라이버
프레임워크(transparent 필터 + per-volume 암복호 스레드 + sweep), UEFI 로더(BlockIo 후킹 + handover),
Go SDK/CLI 모두 동작. 남은 스텁은 async executor(미사용), EFI_BLOCK_IO2 후킹 정도.
아래 각 항목은 구체적인 파일·함수를 가리킵니다.
...
>   크레이트(`vck-sample-driver`)를 통해, **msys2 로그인 셸 + WEDK**에서:
>   `C:/msys64/usr/bin/bash.exe -lc 'cd /d/workspace/volumecrypt-kit; make build-driver'`.
> - `make build-driver`는 **빌드+서명까지 통과**합니다(현재 검증됨).
...
>   드라이버 전용 플래그(`-C target-feature=+crt-static`)는 Makefile의 인라인 RUSTFLAGS로, `driver_model` cfg는
>   각 드라이버 크레이트의 `build.rs`(`wdk_build::configure_wdk_binary_build`)가 방출합니다.
> - **AES-NI 주의**: 드라이버/로더 RUSTFLAGS에 `-C target-feature=+aes`를 **넣지 마세요**. 언롤된 AES-NI가 깊은
>   콜체인에 인라인되어 커널 스택을 넘겨 `0x7F` double-fault(스택 오버플로)를 냅니다. `aes` 크레이트가
>   런타임(CPUID)으로 HW AES를 감지·디스패치하므로 플래그 없이도 하드웨어 AES를 씁니다.
...
| `make build-loader` | `vck-sample-loader` → `.efi` | `x86_64-unknown-uefi` (RUSTFLAGS `-C target-feature=-soft-float`, AES-NI 런타임 감지) — **현재 통과** |
| `make test-vm-os-encrypt` / `test-vm-os-handover` | OS 볼륨 암호화·부팅 / handover E2E | win11 VM — **현재 통과** |
| `make build-app` | `vck-app.exe` (Go) | 호스트 — **현재 통과** |
| `make test-vm-driver-load` | test-foundry VM | win11 VM |

> 커널/UEFI 크레이트는 호스트에서 빌드되지 않습니다(wdk-sys / uefi 타깃). 순수 로직은 `vck-common`
> (std)에서 `cargo test`로 검증하고, 커널 동작은 `vck-sample-driver`로 VM에서 검증합니다.

---

## 0. 선행 작업 (cross-cutting, 다른 작업의 전제)

- [x] **커널 global allocator**: `sample/windrv/src/lib.rs`에
  `#[global_allocator]` 설정 완료(`wdk-alloc::WdkAllocator`). 드라이버 crate에
  `wdk-build`/`wdk-alloc` 의존성과 `package.metadata.wdk.driver-model = "WDM"` 추가,
  `.cargo/config.toml`/`Makefile` WEDK 빌드 경로 정리. `make build-driver` 서명까지 통과.
...
## 1. JVCK 암호 프리미티브 — `lib/common` (✅ 완료, 호스트 테스트 31개 통과)
...
- [x] `lib/common/src/xts.rs` — `XtsVolumeCipher`(공유 AES-256-XTS). 8블록 병렬 경로
  (`encrypt_blocks`→AES-NI `encrypt8` 배치)로 처리량 개선 + 커널 스택 안전(`#[inline(never)]`, cipher 2개만 보관).
- [x] `lib/common/src/cpu.rs` — `has_aes_ni()`(CPUID.01H:ECX[25]), 드라이버·로더 공용.
...
- [x] (uefi feature) `lib/common/src/jvck/store.rs`의 `UefiBlockIoVolume`(`SectorIo`) +
  `open_volume_footer_uefi` 구현 완료 — 로더가 EFI_BLOCK_IO로 footer를 읽어 메타데이터/FVEK를
  복호화. `make test-vm-os-encrypt` 부팅 윈도우 복호화 경로로 검증.
...
- [x] **READ/WRITE crypto 가로채기 구현 완료** — `lib/windrv/src/filter/volume_thread.rs`:
  cipher-bound 필터는 READ/WRITE IRP를 per-volume 시스템 스레드 큐에 넣고 STATUS_PENDING 반환.
  스레드가 own NonPagedPool 버퍼로 하위 동기 IRP를 수행 → READ는 복호화 후 원본 MDL로 복사,
  WRITE는 복사본 암호화 후 하위 기록(`pipeline::CryptoPipeline` 사용). cipher 없는 볼륨은
  `filter/irp.rs::pass_through`로 그대로 통과. `make test-vm-os-encrypt`로 런타임 복호화 검증.
  (`lib/windrv/src/filter/irp.rs`의 doc 주석은 갱신 필요 — pass-through 설명만 남아 있음.)
- [~] `lib/windrv/src/executor.rs::KernelExecutor` — **미사용 스텁**(여전히 `todo!()`). 동기 IRP +
  per-volume 스레드 모델로 전환하면서 async 실행기는 불필요해짐. 제거 또는 보류 결정 필요.
...
- [x] **암호화 sweep worker** 구현 완료 — [filter/volume_thread.rs](lib/windrv/src/filter/volume_thread.rs).
  아키텍처 변경: 전역 단일 스레드(`sweep.rs`)가 아니라 **cipher-bound 필터마다 per-volume 시스템 스레드**
  (`PsCreateSystemThread`)가 bind 시 생성/rebind 시 swap/detach 시 정지. 한 스레드가 그 볼륨의 user IRP
  (암복호)와 sweep 배치(1MiB)를 **직렬로** 처리하므로 sweep↔IRP 레이스가 구조적으로 없음.
  `AttachedVolume::sweep_step`(→`EncryptionEngine::progress_step`)이 배치 + offset 영속화. `START_ENCRYPT`/
  `DECRYPT`는 상태 변경 후 스레드 wake. transparent 필터 위에서 동작하므로 lock/dismount 불필요(라이브 볼륨 암호화).
...
- [x] `lib/windrv/src/handover.rs::read_handover::<P>()` — 페이로드 타입의 `P::VAR_NAME`/`P::VAR_GUID`
  UEFI 변수를 `ExGetFirmwareEnvironmentVariable`로 읽어 `decode_payload`로 역직렬화. plaintext VMK를 담은
  커널 풀 버퍼는 owned `Vec`로 복사 후 zeroize+free.
...
- [x] `lib/loader/src/handover.rs::install_handover` — **UEFI 런타임 변수**(payload 타입의
  `VAR_NAME`/`VAR_GUID`, sample = `VckHandover`)에 msgpack payload를 `SetVariable`
  ...
  > 과거 ACPI 테이블/XSDT 주입 코드(`AcpiHandover{Writer,Reader}`)는 **제거됨**. **VM 부팅 검증 완료(3h).**
...
- [x] **AES-NI 진단 + SSE/XMM 활성화 로깅**: 드라이버(`DriverEntry`)와 로더(`run` 시작)에서
  `vck_common::cpu::has_aes_ni()`로 AES-NI 지원 여부를 로그. 로더는 추가로 `lib/loader/src/cpu.rs`에서
  현재 CR0/CR4 값을 출력하고 AES-NI 지원 시 `CR0.MP=1, CR0.EM=0, CR4.OSFXSR=1, CR4.OSXMMEXCPT=1`로
  설정(멱등, ring-0 전용). VM 검증: 로더가 `CR0=0x80010033 CR4=0x668`(이미 설정됨) 확인.
- [x] **디버그 로깅 통합**: 드라이버 `driver_print!`/`driver_println!`와 로더 `loader_dbg!`를 단일
  매크로 `vck_log!`로 통일. 출력 형식도 `{ts} vck-driver: …` / `{ts} vck-loader: …`로 일치
  (`lib/windrv/src/debug.rs`, `lib/loader/src/debug.rs`).
...
## 5. Go SDK / CLI — `sdk`, `sample/app` (대체로 완료)

빌드·`go vet` 통과. 남은 항목:

- [x] `bench-aes` CLI(`sample/app/cmd/bench.go`) — `IOCTL_VCK_BENCH_AES`로 커널 내 AES-256-XTS
  암/복호 throughput(MiB/s) 측정, 시작 시 AES-NI 지원 여부 출력.
- [x] `IOCTL_VCK_LIST_VOLUMES` + `volume-info`/`status`(`sdk/client.go::ListVolumes`,
  `sample/app/cmd/volume_info.go`) — 부착된 볼륨 열거(입력 없이 드라이버 연결 확인 겸용).
...
- [x] test recipe 존재: `driver-load`, `driver-load-dev`, `data-volume`, `os-volume-prepare`,
  `os-handover`, `os-encrypt`, `os-encrypt-dev`.
- [ ] 부재 recipe: `testing/recipes/smoke-guest-exec/smoke.yaml` (Makefile `test-vm-smoke`가 참조).
- [x] `testing/images/make-volume-d.ps1`로 만든 `D:\`(10GB)에서 Data Volume attach→encrypt→상태
  end-to-end 시나리오 recipe(`test-vm-data-volume`, `testing/recipes/data-volume/`).
...
1. **§1** JVCK 프리미티브(호스트 `cargo test`로 즉시 검증) → 2. **§2** 드라이버 프레임워크
3. **§4** 샘플 드라이버 + `test-vm-driver-load`/`test-vm-data-volume` → 4. **§3** UEFI 로더
5. **§6** OS Volume 부팅 end-to-end.
§0 항목은 각 단계 진입 전 전제로 처리하세요.

> **진행 현황**: §0–§4, §5, §6의 OS 볼륨 부팅 E2E까지 구현·VM 검증 완료. 남은 작업은
> 부재 recipe(smoke), 미사용 async executor 정리, EFI_BLOCK_IO2 후킹(선택),
> hard-crash crash-consistency(샘플 범위 밖).
