...
type JvckVolumePrepareRequest struct {
	...
	// Pre-encoded 512-byte JVCK Metadata block generated by the app with
	// cryptographically random FVEK + volume ID. The driver writes this to every
	// replica LBA while the volume lock is held (before NTFS re-mounts), so NTFS
	// can never overwrite the metadata region. Empty means re-attach (skip write).
