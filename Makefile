...
.PHONY: build-common build-driver build-crypto-test-driver build-loader build-app test $(TEST_VM_DIR) test-vm-smoke test-vm-driver-load test-vm-data-volume test-vm-crypto-test clean
...
build-driver: testing/signing/MyTestDriverCert.cer
	MSYS2_ARG_CONV_EXCL="/c" cmd.exe /c "call G:\\BuildEnv\\SetupBuildEnv.cmd && cd /d . && set RUSTFLAGS=-C panic=abort -C target-feature=+crt-static --cfg aes_force_soft && cargo build -p vck-sample-driver --target x86_64-pc-windows-msvc"
...
build-crypto-test-driver: testing/signing/MyTestDriverCert.cer
	MSYS2_ARG_CONV_EXCL="/c" cmd.exe /c "call G:\\BuildEnv\\SetupBuildEnv.cmd && cd /d . && set RUSTFLAGS=-C panic=abort -C target-feature=+crt-static --cfg aes_force_soft && cargo build -p vck-crypto-test-driver --target x86_64-pc-windows-msvc"
