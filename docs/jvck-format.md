...
| 176 | 32  | HMAC-SHA256(key = MAC_KEY, data = Encrypted Metadata) |
...
```text
MAC_KEY = HKDF_SHA256(salt = Volume ID, ikm = VMK, info = "EncryptedMetadata:MAC", len = 32)
ENC_KEY = HKDF_SHA256(salt = Volume ID, ikm = VMK, info = "EncryptedMetadata:ENC", len = 32)
ENC_IV  = HKDF_SHA256(salt = Volume ID, ikm = VMK, info = "EncryptedMetadata:IV",  len = 16)
```
