...
- `IoConfig` — `Passthrough` / `AesXts{key1,key2,offset_sector,encrypted_offset,offset_store}` /
  `Custom{io_hooks,...}`(저수준).
- `EncryptionEngine`(`offset/engine.rs`) — 점진적 암복호 상태 머신 + 배치 sweep + offset 영속화.
- `VolumeProvider` 트레이트 — OS 볼륨 attach 콜백(`on_attach` → `IoConfig` 반환). 연관 타입
  `Payload: HandoverPayload`로 핸드오버를 구체 타입으로 받습니다(`dyn Any` downcast 불필요).
...
- `LoaderProvider` 트레이트 — `on_init()`이 `LoaderConfig{handover_payload, next_loader, crypto}`를 반환.
...
- **최소 sample**: I/O 라우팅·암복호·핸드오버 발행/읽기는 전부 `lib`. sample은 트레이트 구현·설정만.
- **언어 경계**: 커널/UEFI는 Rust+WDK, 관리 도구는 Go. 경계는 `DeviceIoControl` + msgpack.
- **두 I/O 경로**: `AesXts`(고수준, 키트가 암복호) / `Custom`(저수준, sample이 섹터 단위 직접 처리).
